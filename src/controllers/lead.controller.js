...
export const createLead = async (req, res) => {
    try {
        const lead = await leadService.createLead(req.body, req.ownerId);
...
export const updateLead = async (req, res) => {
    try {
        const lead = await leadService.updateLead(req.params.id, req.body, req.ownerId);
        if (!lead) {
...
export const getLeads = async (req, res) => {
    try {
        const leads = await leadService.getLeads(req.ownerId);
        return res.status(200).json({
            success: true,
...
export const getLeadbyId = async (req, res) => {
    try {
        const lead = await leadService.getLeadById(req.params.id, req.ownerId);
        if (!lead) {
            return res.status(404).json({
                success: false,
                message: 'Lead not found or unauthorized'
...
export const deleteLeadbyId = async (req, res) => {
    try {
        const lead = await leadService.deleteLead(req.params.id, req.ownerId);
        if (!lead) {
            return res.status(404).json({
                success: false,
                message: 'Lead not found or unauthorized'
...
export const convertLeadToCustomer = async (req, res) => {
    try {
        ...
        await Promise.all([
            leadService.convertLeadToCustomer(req.params.id, req.ownerId),
            clientService.createClient(clientData, req.ownerId)
        ]);
...
export const addActivity = async (req, res) => {
    try {
        ...
        const lead = await leadService.addActivityToLead(id, activity, req.ownerId);
        if (!lead) {
            return res.status(404).json({
                success: false,
                message: 'Lead not found or unauthorized'
...
export const deleteActivity = async (req, res) => {
    try {
        ...
        const lead = await leadService.deleteActivityFromLead(id, activity, req.ownerId);
        if (!lead) {
            return res.status(404).json({
                success: false,
                message: 'Lead not found or unauthorized'
